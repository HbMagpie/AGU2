...
<h4>이용 약관</h4>
제1조(목적)
이 약관은 AGU(전자상거래 사업자)(이)가 운영하는 AGU(이하 "몰"이라 한다)에서 제공하는 인터넷 관련 서비스(이하 "서비스"라 한다)를 이용함에 있어 사이버 몰과 이용자의 권리·의무 및 책임사항을 규정함을 목적으로 합니다. ※「PC통신, 무선 등을 이용하는 전자상거래에 대해서도 그 성질에 반하지 않는 한 이 약관을 준용합니다」
제2조(정의)
① "몰" 이란 AGU(이)가 재화 또는 용역(이하 "재화등"이라 함)을 이용자에게 제공하기 위하여 컴퓨터등 정보통신설비를 이용하여 재화등을 거래할 수 있도록 설정한 가상의 영업장을 말하며, 아울러 사이버몰을 운영하는 사업자의 의미로도 사용합니다.
...
제15조(청약철회 등)
① "몰"과 재화등의 구매에 관한 계약을 체결한 이용자는 수신확인의 통지를 받은 날부터 7일 이내에는 청약의 철회를 할 수 있습니다. 상품에 하자가 있는 경우에는 그 사실을 안 날 또는 알 수 있었던 날부터 30일 이내에 청약 철회를 할 수 있습니다.
...
제25조(적립금/예치금 제도)
① 적립금과 예치금은 "몰"내에서 현금처럼 사용되는 전자화폐이며, 상품 구매 시에만 사용할 수 있습니다. 단, 적립금 사용시 최소구매가능적립금(구매가능한 적립금 요구선)은 KRW 1,000 입니다.
② 적립금과 예치금은 본인만 사용 가능하며 타인에게 양도 또는 다른 아이디와 연동할 수 없습니다.
③ 적립금과 예치금은 회원에 한하여 제공되는 서비스이며 적립금은 현금 환불은 되지 않습니다.
...
⑤ 회원에게 적립된 예치금 또는 적립금이 부정한 방법 또는 개인 정보 도용으로 적립된 사실이 인정되는 경우 몰은 해당 포인트를 통보 없이 정정 또는 회수할 수 있습니다.
⑥ 회원이 상품 구매나 이벤트 등을 통하여 “몰”로부터 무상 제공받은 적립금의 경우 "몰"에서 정한 기한에 한해 사용 가능합니다.
⑦ 유효기간 : “몰”로부터 제공받은 적립금의 경우, 유효기간이 경과한 적립금은 자동 소멸됩니다.
⑧ 적립금 소멸일 기준으로 1년 이상 경과된 적립금은 회수됩니다. 적립금의 유효기간은 지급 시점을 기준으로 12개월이며, 유효기간 내에 사용되지 않은 적립금은 기간 만료 시 자동 소멸됩니다.
⑨ 이용자가 탈퇴, 자격상실, 1년 이상 무거래(휴면 계정 전환) 등을 원인으로 적립금이 소멸되는 경우 현금으로 반환되지 않고 소멸되며, 소멸된 적립금은 복구가 불가능합니다.
변경이력
이 약관은 2023 년 6 월 1일부터 시행합니다.
